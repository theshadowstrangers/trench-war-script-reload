...
espPlayersBtn.Parent = espTab
Instance.new("UICorner", espPlayersBtn).CornerRadius = UDim.new(0, 4)

local espCarActive = false
local espCarObjects = {}
local espCarConnections = {}

local espCarBtn = Instance.new("TextButton")
espCarBtn.Size = UDim2.new(1, 0, 0, 32)
espCarBtn.Text = "Esp Car OFF"
espCarBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
espCarBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
espCarBtn.Font = Enum.Font.SourceSansBold
espCarBtn.TextSize = 14
espCarBtn.BorderSizePixel = 0
espCarBtn.Parent = espTab
Instance.new("UICorner", espCarBtn).CornerRadius = UDim.new(0, 4)
...
local function clearESPCar()
    for _, obj in pairs(espCarObjects) do
        obj:Destroy()
    end
    espCarObjects = {}
    for _, conn in pairs(espCarConnections) do
        conn:Disconnect()
    end
    espCarConnections = {}
end
...
local function createPlayerESP(player)
    ...
    local connection = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        distLabel.Text = "studs: " .. math.floor(dist)
    end)
...
local function createCarESP()
    clearESPCar()
    
    local carPart = workspace:FindFirstChild("Vehicles")
    if not carPart then return end
    
    local car = carPart:FindFirstChild("Claptima")
    if not car then return end
    
    local geo = car:FindFirstChild("geo")
    if not geo then return end
    
    local body = geo:FindFirstChild("body")
    if not body then return end
    
    local paint = body:FindFirstChild("paint")
    if not paint or not paint:IsA("BasePart") then return end
    ...
    highlight.OutlineTransparency = 0.2
    highlight.Parent = paint
    
    local billboard = Instance.new("BillboardGui")
    billboard.Name = "CarESP_Billboard"
    billboard.Size = UDim2.new(0, 200, 0, 30)
    billboard.AlwaysOnTop = true
    billboard.Parent = paint
    ...
    table.insert(espCarObjects, highlight)
    table.insert(espCarObjects, billboard)
    
    local connection = game:GetService("RunService").Heartbeat:Connect(function()
        if not espCarActive then
            connection:Disconnect()
            return
        end
        if not paint or not paint.Parent then
            connection:Disconnect()
            return
        end
        local localChar = LocalPlayer.Character
        ...
        local dist = (paint.Position - localHrp.Position).Magnitude
        distLabel.Text = "studs: " .. math.floor(dist)
    end)
    table.insert(espCarConnections, connection)
end
...
espPlayersBtn.MouseButton1Click:Connect(function()
    ...
    if espPlayersActive then
        ...
        espPlayersBtn.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
        ...
        local conn2 = game:GetService("Players").PlayerRemoved:Connect(function()
            if espPlayersActive then
                task.wait(0.1)
                scanPlayers()
            end
        ...
        local conn3 = game:GetService("Players").PlayerAdded:Connect(function(player)
...
espCarBtn.MouseButton1Click:Connect(function()
    espCarActive = not espCarActive

    if espCarActive then
        espCarBtn.Text = "Esp Car ON"
        espCarBtn.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
        espCarBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
        createCarESP()
    else
        espCarBtn.Text = "Esp Car OFF"
        espCarBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
...
-- ==================== TELEPORT ====================
-- Tp to 50%
local tp50Btn = Instance.new("TextButton")
tp50Btn.Size = UDim2.new(1, 0, 0, 32)
tp50Btn.Text = "Tp to 50%"
tp50Btn.TextColor3 = Color3.fromRGB(255, 255, 255)
tp50Btn.BackgroundColor3 = Color3.fromRGB(200, 100, 50)
tp50Btn.Font = Enum.Font.SourceSansBold
tp50Btn.TextSize = 14
tp50Btn.BorderSizePixel = 0
tp50Btn.Parent = teleportTab
Instance.new("UICorner", tp50Btn).CornerRadius = UDim.new(0, 4)

tp50Btn.MouseButton1Click:Connect(function()
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if hrp then
        hrp.CFrame = CFrame.new(-314.442047, 1953.1355, 34366.5625, -0.940875113, 1.44936618e-09, 0.33875367, 9.83910398e-10, 1, -1.54575275e-09, -0.33875367, -1.12105702e-09, -0.940875113)
    end
end)

-- Tp to 99%
local tp99Btn = Instance.new("TextButton")
tp99Btn.Size = UDim2.new(1, 0, 0, 32)
tp99Btn.Text = "Tp to 99%"
tp99Btn.TextColor3 = Color3.fromRGB(255, 255, 255)
tp99Btn.BackgroundColor3 = Color3.fromRGB(200, 50, 100)
tp99Btn.Font = Enum.Font.SourceSansBold
tp99Btn.TextSize = 14
tp99Btn.BorderSizePixel = 0
tp99Btn.Parent = teleportTab
Instance.new("UICorner", tp99Btn).CornerRadius = UDim.new(0, 4)

tp99Btn.MouseButton1Click:Connect(function()
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if hrp then
        hrp.CFrame = CFrame.new(976.696106, 1861.38843, 83691.8359, 0.78896594, 0.13634938, -0.599117339, 3.06861648e-06, 0.975066364, 0.221913338, 0.614436924, -0.175083905, 0.769293785)
    end
end)

-- Tp to car
local tpCarBtn = Instance.new("TextButton")
tpCarBtn.Size = UDim2.new(1, 0, 0, 32)
tpCarBtn.Text = "Tp to car"
tpCarBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
tpCarBtn.BackgroundColor3 = Color3.fromRGB(50, 150, 200)
tpCarBtn.Font = Enum.Font.SourceSansBold
tpCarBtn.TextSize = 14
tpCarBtn.BorderSizePixel = 0
tpCarBtn.Parent = teleportTab
Instance.new("UICorner", tpCarBtn).CornerRadius = UDim.new(0, 4)

tpCarBtn.MouseButton1Click:Connect(function()
    local carPart = workspace:FindFirstChild("Vehicles")
    if not carPart then return end
