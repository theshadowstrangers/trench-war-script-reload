...
local killersBtn, killersTab = createTab("Killers", "Killers")
...
infoLabel.Text = "Area51 Classic 2018\nBy Spynote\nCreate in 2026 year\nalltabs Teleport, Exploit, Misc, Guns, Badges, Esp, Killers"
...
local exploitBtnToggle = Instance.new("TextButton")
exploitBtnToggle.Size = UDim2.new(1, 0, 0, 35)
exploitBtnToggle.Text = "Exploit Caboom OFF"
exploitBtnToggle.TextColor3 = Color3.fromRGB(255, 85, 85)
exploitBtnToggle.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
exploitBtnToggle.Font = Enum.Font.SourceSansBold
exploitBtnToggle.TextSize = 14
exploitBtnToggle.BorderSizePixel = 0
exploitBtnToggle.Parent = exploitTab
Instance.new("UICorner", exploitBtnToggle).CornerRadius = UDim.new(0, 4)

exploitBtnToggle.MouseButton1Click:Connect(function()
    isExploding = not isExploding
    if isExploding then
        exploitBtnToggle.Text = "Exploit Caboom ON"
        exploitBtnToggle.BackgroundColor3 = Color3.fromRGB(85, 255, 85)
        exploitBtnToggle.TextColor3 = Color3.fromRGB(0, 0, 0)
        startExploit()
    else
        exploitBtnToggle.Text = "Exploit Caboom OFF"
        exploitBtnToggle.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
...
-- ==================== KILLERS ====================
local isKillerEspEnabled = false
local killerHighlights = {}
local killerBillboards = {}
local isHitboxEspEnabled = false
local hitboxSizeValue = 20

local function clearKillerEsp()
    for _, item in pairs(killerHighlights) do if item then item:Destroy() end end
    killerHighlights = {}
    for _, bill in pairs(killerBillboards) do if bill then bill:Destroy() end end
    killerBillboards = {}
end
...
local function updateKillerEsp()
    if not isKillerEspEnabled then
        clearKillerEsp()
        return
    end

    for _, monster in pairs(getKillers()) do
        local zombie = monster:FindFirstChild("Zombie") or monster
        local root = zombie:IsA("BasePart") and zombie or zombie:FindFirstChild("HumanoidRootPart") or zombie:FindFirstChildWhichIsA("BasePart")
        if root then
            local highlight = monster:FindFirstChild("SpyNoteKillerESP")
            if not highlight then
                highlight = Instance.new("Highlight")
                highlight.Name = "SpyNoteKillerESP"
            ...
            local bill = root:FindFirstChild("SpyNoteKillerText")
            if not bill then
                bill = Instance.new("BillboardGui")
                bill.Name = "SpyNoteKillerText"
                bill.Size = UDim2.new(0, 80, 0, 25)
                bill.StudsOffset = Vector3.new(0, 3, 0)
                bill.AlwaysOnTop = true
                ...
                txt.Parent = bill
                bill.Parent = root
                table.insert(killerBillboards, bill)
            end
...
hitboxLabel.Parent = killersTab

local hitboxInput = Instance.new("TextBox")
hitboxInput.Size = UDim2.new(1, 0, 0, 25)
hitboxInput.PlaceholderText = "20"
hitboxInput.Text = "20"
hitboxInput.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
hitboxInput.TextColor3 = Color3.new(1, 1, 1)
hitboxInput.Font = Enum.Font.SourceSans
hitboxInput.TextSize = 13
hitboxInput.Parent = killersTab
Instance.new("UICorner", hitboxInput).CornerRadius = UDim.new(0, 4)

local hitboxUsingBtn = Instance.new("TextButton")
hitboxUsingBtn.Size = UDim2.new(1, 0, 0, 25)
hitboxUsingBtn.Text = "Hitbox Using"
hitboxUsingBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
hitboxUsingBtn.BackgroundColor3 = Color3.fromRGB(50, 100, 200)
hitboxUsingBtn.Font = Enum.Font.SourceSansBold
hitboxUsingBtn.TextSize = 13
hitboxUsingBtn.BorderSizePixel = 0
hitboxUsingBtn.Parent = killersTab
Instance.new("UICorner", hitboxUsingBtn).CornerRadius = UDim.new(0, 4)

hitboxUsingBtn.MouseButton1Click:Connect(function()
    local num = tonumber(hitboxInput.Text)
    if num and num > 0 then
        hitboxSizeValue = num
        hitboxInput.Text = tostring(num)
    else
        hitboxInput.Text = tostring(hitboxSizeValue)
    end
end)

-- Hitbox Esp Toggle
local hitboxEspBtn = Instance.new("TextButton")
hitboxEspBtn.Size = UDim2.new(1, 0, 0, 32)
hitboxEspBtn.Text = "Killers Hitbox Esp OFF"
hitboxEspBtn.TextColor3 = Color3.fromRGB(255, 85, 85)
hitboxEspBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
hitboxEspBtn.Font = Enum.Font.SourceSansBold
hitboxEspBtn.TextSize = 13
hitboxEspBtn.BorderSizePixel = 0
hitboxEspBtn.Parent = killersTab
Instance.new("UICorner", hitboxEspBtn).CornerRadius = UDim.new(0, 4)

hitboxEspBtn.MouseButton1Click:Connect(function()
    isHitboxEspEnabled = not isHitboxEspEnabled
    if isHitboxEspEnabled then
        hitboxEspBtn.Text = "Killers Hitbox Esp ON"
        hitboxEspBtn.BackgroundColor3 = Color3.fromRGB(85, 255, 85)
        hitboxEspBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
        task.spawn(function()
            while isHitboxEspEnabled do
                for _, monster in pairs(getKillers()) do
                    local zombie = monster:FindFirstChild("Zombie") or monster
                    local root = zombie:IsA("BasePart") and zombie or zombie:FindFirstChild("HumanoidRootPart") or zombie:FindFirstChildWhichIsA("BasePart")
                    if root then
...
-- TP to Random Killer
local tpRandomKillerBtn = Instance.new("TextButton")
tpRandomKillerBtn.Size = UDim2.new(1, 0, 0, 32)
tpRandomKillerBtn.Text = "TP to Random-Killer"
tpRandomKillerBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
tpRandomKillerBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 50)
tpRandomKillerBtn.Font = Enum.Font.SourceSansBold
tpRandomKillerBtn.TextSize = 13
tpRandomKillerBtn.BorderSizePixel = 0
tpRandomKillerBtn.Parent = killersTab
Instance.new("UICorner", tpRandomKillerBtn).CornerRadius = UDim.new(0, 4)

tpRandomKillerBtn.MouseButton1Click:Connect(function()
    local monsters = getKillers()
    if #monsters == 0 then return end
    local randomMonster = monsters[math.random(1, #monsters)]
    local zombie = randomMonster:FindFirstChild("Zombie") or randomMonster
    local root = zombie:IsA("BasePart") and zombie or zombie:FindFirstChild("HumanoidRootPart") or zombie:FindFirstChildWhichIsA("BasePart")
    if root then
        local char = LocalPlayer.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
...
-- Exploit Caboom (перенесли в Misc)
...
CloseButton.MouseButton1Click:Connect(function()
    ...
    isKillAll = false
    isEspEnabled = false
    isKillerEspEnabled = false
    isHitboxEspEnabled = false
    if loopConnection then loopConnection:Disconnect() end
